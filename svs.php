...
class Scanner
{
	...
	public function __construct($rootFolder = __DIR__)
	{
		...
		if (isset($options['html']))
			$this->isWeb = true; //override with --html command line switch
		if (isset($_GET['plaintext']))
			$this->isWeb = false; //override with URL parameter

		if (isset($options['root']))
			$this->rootFolder = $options['root']; //override with --root command line switch

		if (isset($options['nosort']))
			$this->noSort = true; //override with --nosort command line switch or parameter
		if (isset($_GET['nosort']))
			$this->noSort = true;

		if (isset($options['m']))
			$this->minScore = $options['m']; //set minimal reporting score via command line with --m=5 switch
		if (isset($_GET['m']))
			$this->minScore = $_GET['m']; //set minimal reporting score via URL
