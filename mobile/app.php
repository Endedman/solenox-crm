...
try {
    ...
    $appId = $_GET['id']; // Получаем ID приложения из URL
    $app = $appManager->getApp($appId);
    $screenshots = $appManager->getScreenshotsByAppId($appId);  // вызываем функцию для получения скриншотов
    ...
    <div data-role="page" data-theme="<?php if (isset($_COOKIE['theme'])) {
        ...
        <div data-role="content">
            <ul data-role="listview" data-inset="true">
                ...
                <li>
                    <?= $lang['file_size']; ?>:
                    <?php echo $app['file_size']; ?> bytes
                </li>
                <li>
                    <?= $lang['updated_date']; ?>:
                    <?php echo $app['upload_date']; ?>
                </li>
                ...
                <li>
                    ...
                    foreach ($screenshots as $screenshot) {
                        echo '<img src="' . JSTORE_WEB_UPLOAD_DIR . $screenshot['file_url'] . '" alt="Application Screenshot" style="width: 100%; height: auto;" />';
                    }
                    ?>
                </li>
            </ul>

            <a href="<?php echo JSTORE_WEB_UPLOAD_DIR . $app['filename']; ?>" target="_blank" data-role="button"
                data-inline="true">
                <?= $lang['download']; ?>
            </a>
        </div>
