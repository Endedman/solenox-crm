    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['username'], $_POST['password'])) {
            ...
            if ($result && $result['success']) {
                setcookie("username", $result["user"]["username"], time() + 3600, "/");
                $_SESSION['user_id'] = $result['user']['id'];
                $message = $result ? $result['message'] : "Logged in. <script>window.location.href='//<?= JSTORE_WEB_MOBILE_SHORT_DIR?>';</script>";
            } else {
                $message = $result ? $result['message'] : "Invalid Login credentials";
            }
        } else {
            $message = $result ? $result['message'] : "Please fill all the fields.";
        }
    }
...
<body>
    <div data-role="page" data-theme="<?php if (isset($_COOKIE['theme'])) {
        ...
        <div data-role="main" class="ui-content">
            <form method="post" action="/<?= JSTORE_WEB_MOBILE_SHORT_DIR ?>login">
                <label for="username">
